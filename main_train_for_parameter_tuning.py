...
def main(config):
    ...
    layer1 = 3  # CAB第一层网络块数（论文最优）
    layer2 = 2  # CAB第二层网络块数（论文最优）
    layer3 = 8  # CAB第三层网络块数（论文最优）
    gamma = 0.4  # CMPLM伪注意力衰减率（论文最优）
    
    #for layer1 in layers1:
    #   for layer2 in layers2:
    #        for layer3 in layers3:
    #            for gamma in gammas:
    print(layer1, layer2, layer3, gamma)
...
#    print(subName)
    ...
    main_path = './datasets/three_norm_u_v_os'
    # 只保留你想训练的subject列表（精准控制）
    subName = [
        'sub17', 'sub26', 'sub16', 'sub09', 'sub05', 'sub24', 'sub02', 'sub13', 'sub04', 'sub23',
        'sub11', 'sub12', 'sub08', 'sub14', 'sub03', 'sub19', 'sub01',
        'sub20', 'sub21', 'sub22', 'sub15', 'sub06', 'sub25', 'sub07', 's06', 's08', 's09', 
        's11', 's12', 's13', 's14', 's15', 's18', 's19', 's20'
    ]
    # 可选：过滤掉数据集文件夹里不存在的subject（避免报错）
    all_exist_subjects = os.listdir(main_path)
    subName = [sub for sub in subName if sub in all_exist_subjects]
    all_five_parts_optical_flow = crop_optical_flow_block()
    print("即将训练的subject列表：", subName)
